# 组装中文版带图成稿 DOCX v5（与 JRTIP-paper-v5 逐节对齐：8 图 7 表 + 补充材料表 S1/S2）
# 运行：python scripts/build_paper_docx_zh_v5.py → paper/猪行为检测-中文成稿-v5.docx
# v5 相对 v4：2.1/2.2/2.3 末段补文献（[27]–[33]）、7 讨论补对比段、4.4 补表 S2 指引、
#             关键词 8→6、参考文献 +[27]–[33]、摘要/表3脚注/4.5 节改三种子基线口径
#             （基线 val 0.5822±0.0087 / test 0.6060±0.0084，2026-08-07 AutoDL 封账）、
#             审稿团报告#7 措辞修复同步（统计相当/就我们所知/suggest 软化）
import sys
...
p('群养猪的自动化行为监测，要求检测模型既能识别稀有但关乎福利的行为，又能在低成本边缘硬件上实时运行。'
  '本文基于 YOLOv11n 构建了一套实时多行为检测框架，并在 Jetson Nano 上完成部署验证，三项设计选择'
  '支撑起最终结果：其一，类别不均衡感知采样策略（频率封顶的离线过采样），不改网络即提升稀有类检测——'
  '低频行为 active 的 AP 在测试集上从 0.526 提升至 0.639，总体召回率在验证集上提高 2.1 个百分点；其二，'
  'FasterNet 轻量化主干替换配合"恒等保持"预训练权重迁移策略，参数量降低 4.4%，三次重复实验中精度与'
  '基线统计相当（test mAP50 0.590±0.009 对三种子基线 0.606±0.008），且出人意料地是全部参评模型中最快的'
  '（RTX 3090 上 117.6 FPS）；其三，Jetson Nano 实测验证实时性（640×640 输入 19.7 FPS、480×480 输入 '
  '33.3 FPS、整机总功耗约 7 W（计算轨约 5 W））。两级泛化分析（未见序列压力测试与独立农场数据集）则为整套研究收束闭环：'
  '划清框架在何处有效、何处失效。')
p('关键词：猪行为检测；类别不均衡；FasterNet；边缘计算；Jetson Nano；精准畜牧',
  italic=True)
...
p('但监测要在经济账上成立，检测就必须跑在猪舍内的廉价边缘设备上。一个商业猪场可能有上百个栏位，'
  ...
  '延迟同样关键：一条迟到几分钟的打斗警报几乎没有价值。工程趋势因此只有一个方向——把检测器带到'
  '数据旁边，而不是反过来。')
...
p('（3）Jetson Nano 部署验证（19.7–33.3 FPS、约 7 W 整机功耗）与两级泛化分析（未见序列、独立农场数据集），'
  '完整划定框架的适用边界。')
...
p('YOLO 检测器已应用于猪只检测、个体追踪、姿态识别与行为分析。对群养猪，Tu 等 [5] 以 '
  ...
  '最近的两项研究以不同侧重推进了同一部署议程。PBR-YOLO [27] 将 GhostNet 主干、融合高效多尺度注意力的 '
  'FasterNet 块与轻量多路检测头组合，用于八类仔猪行为，在 Jetson Orin NX 上以 TensorRT 报告每帧 '
  '15.8 ms——一种注意力增强设计，但其验证板卡远强于我们目标的 Nano 级硬件。Rahman 等 [28] 针对产房母猪'
  '哺乳这一单一任务裁剪 YOLO11n（移除小目标检测头）并以 TensorRT 加速，在随机图像级切分上评估——'
  '他们自己明确将其定位为栏内性能。类别不均衡治理是我们场景的核心，但不属于这两种方案的配方；'
  '就我们所知，两者均未报告稀有行为的分类别精度，也未报告对未见序列的泛化。')
h('2.2 目标检测中的类别不均衡', 2)
p('前景类不均衡的治理路线大致三家：采样（类别感知采样、重复因子采样 [8]）、损失重加权'
  ...
  '实例感知的重复因子采样 [29] 以标注实例数改进 RFS；我们则按含类图像计数——复制操作实际复制的正是'
  '这个单元——并加开方阻尼与硬封顶，使少数稀有场景无法主导一个训练轮。针对单阶段检测器治理路线的'
  '系统性诊断 [30] 同样发现没有普遍占优的策略家族，这支持我们选择侵入性最低的那一条。')
h('2.3 轻量化检测与边缘部署', 2)
p('相关轻量化设计包括深度可分离卷积（MobileNet [14]）、特征复用卷积（GhostNet [15]）与部分卷积'
  ...
  'FasterNet 式替换本身已有先例——Guo 等 [31] 将 FasterNet 集成进 YOLOv8 用于实时水下检测，'
  'YOLO-FGD [32] 将同一思路用于 PCB 缺陷检测，两者均发表在本刊——因此我们的贡献不在替换本身，'
  '而在于替换保持精度的条件：恒等保持的权重迁移（3.5 节），这是上述工作未讨论的。另一条互补的轻量化'
  '路线是训练后压缩：Luo 等 [33] 将通道剪枝与知识蒸馏结合用于仔猪行为识别，以小幅精度裕量换取大幅'
  '参数削减，且不改动训练期架构。')
...
p('实验采用公开群养猪行为数据集（Roboflow Universe，CC BY 4.0 [19]），源自 Bergamini 等 [20] 公开记录的'
  ...
  '（训练 3,936 / 验证 1,123 / 测试 561 张）。行为定义与标注协议遵循原始采集研究 [20]；标注质量另经'
  '训练集随机抽样目检核验。本研究仅使用公开获取的'
  '非侵入式数据，不涉及动物实验，无需伦理审批。表 1 给出各类的行为学定义并汇总实例分布，'
  '图 1 将分布与由其导出的复制倍率（见 3.3 节）一并可视化。')
table(['类别', '行为学定义与管理意义', '实例数†', '训练图像数‡'],
      [['active', '高活动量运动：奔跑、跳跃（由低频 run/jump 类聚合）。活动异常指示应激或疾病。', '259', '167'],
       ['drink', '头部位于饮水器处并呈饮水姿态。饮水异常信号消化/泌尿系统问题。', '211', '147'],
       ['eat', '头部入饲槽并持续采食。采食量是健康/生长核心指标。', '980', '577'],
       ['fight', '攻击性接触：两头及以上个体撕咬、追逐、顶撞。福利预警关键行为，混群期高发。', '807', '354'],
       ['investigating', '探究：拱地、嗅探地面/栏体/同伴（非采食）。正常探究行为，反映环境丰富度。', '4,203', '1,714'],
       ['lying', '侧躺或俯卧，躯干完全着地，静止休息。舒适度/健康直接指标。', '2,485', '1,299'],
       ['nose-to-nose', '两头猪鼻部直接接触且无攻击动作。社会交往，群体关系指标。', '358', '181'],
       ['sitting', '犬坐姿：前腿直立、后躯着地。偶见姿态，可能关联跛行/不适。', '144', '104'],
       ['standing', '四肢站立且无位移。基础姿态行为。', '1,812', '986'],
       ['walk', '四肢行走的位移运动（不含奔跑）。日常活动量组成。', '2,736', '1,259']],
      '表 1 行为类别、行为学定义与实例分布。†全集实例统计（合计 13,995）；'
      '‡训练集中含该类图像数（3.3 节复制倍率的计算基准）。')
...
p('我们不对损失函数重加权——那会改变优化图景并可能与预训练权重相互干扰——而是调整训练数据的采样'
  ...
  '表 2 报告封顶值的敏感性：本数据集封顶 4 与 5 一致，封顶 3 时 sitting 曝光不足。')
table(['封顶值', 'val mAP50', 'test mAP50', '训练集规模'],
      [['3', '0.5754', '0.5776', '5,785'],
       ['4', '0.5816', '0.6035', '5,889'],
       ['5', '0.5816', '0.6035', '5,889']],
      '表 2 复制封顶值敏感性。本数据集实际用到的最大倍率为 4（sitting），故封顶 4 与 5 一致；'
...
h('4.2 消融实验', 2)
p('表 3 按我们实际探索的顺序走过整个设计空间，包括失败。两次注意力集成尝试（M1、M2）列在最前：'
  ...
  '重采样反而致其过拟合的证据；它也回溯性地支持了我们在这一"数据集+方法"组合下选择 YOLOv11n 作基座'
  '的经验判断。')
table(['模型', '改动', 'mAP50', 'mAP50-95', '参数量', '判定'],
      ...
      '表 3 独立测试集上的消融。*M1/M2 被提前否决，其数值为验证集 mAP50；其余各行为单次实验 test 值。'
      '统计参照：三次重复 test 均值为 0.5987±0.0062（M4）、0.5904±0.0086（M5），三种子基线为 '
      '0.6060±0.0084——即表中单次值落在运行间噪声带内，并非挑选有利种子（见 4.5 节）。'
      'M4 的 +2.1 点召回增益测自验证集。')
fig('results/analysis/fig5-curves.png',
    '图 3 训练过程验证集 mAP50 曲线。采样变体（M4）自早期轮次即领先；组合变体（M5）紧贴基线；'
    '早停（patience 30）在 M4 峰值后兜住其轻微过拟合')
h('4.3 与主流模型对比', 2)
p('表 4 在相同训练条件下对比两个采用变体与对照组。M4（0.6035）超过 YOLOv5n（0.6001）、RT-DETR-l'
  ...
  '两个现象尤为突出：其一，YOLOv8n 在验证集上高于我们基线、在测试集上反而更低——验证集排名并不总能'
...
table(['模型', 'test mAP50', 'test mAP50-95', '参数量', 'GFLOPs'],
      ...
      '表 4 同一协议下的主流检测器对比（按 test mAP50 排序）。所有模型均从官方预训练权重训练。')
h('4.4 分类别分析', 2)
p('整体 mAP 掩盖了这套框架存在的理由，在线资源 1（补充材料）表 S1 因此在验证集上按行为类别拆分 AP50——'
  '四个消融模型在验证集上可直接互比。M4 的增益精确落在预定目标上：active 由 0.459 升至 0.552（+9.3 点），drink 由 0.408 '
  ...
  '基线、M4 与 M5 在独立测试集上的分类别 AP50 见在线资源 1 表 S2；稀有类格局与验证集一致。'
  '部署配对的 PR 曲线见补充材料图 S1。')
h('4.5 统计可靠性', 2)
p('小型检测器单次训练的波动轻易可达半个百分点，因此基线与两个采用模型都以三个不同种子重复。'
  '基线验证集 0.5822±0.0087、测试集 0.6060±0.0084（种子 0 精确复现表 3 所报的 0.5964 单次实验）。'
  'M4 验证集 0.5790±0.0054、测试集 0.5987±0.0062；M5 分别为 0.5620±0.0079 与 0.5904±0.0086。'
  '相对三种子基线 test 均值，M5 的差值（-0.016）约相当于种子间变异的两个标准误，在 0.05 水平不显著'
  '（双样本 t 检验，p≈0.09），故我们将 M5 的精度表述为与基线统计相当而非超越。'
  'M4 的 test 均值落在基线一个标准差以内（-0.007）——其真正价值如 4.4 节所示，在于精度向稀有的福利相关行为'
  '再分配，以及验证集 +2.1 点的召回增益，而非整体 mAP 上涨。我们刻意这样呈现："整体提升"的框架会夸大'
  '证据，而稀有类框架既诚实，对福利预警又恰是实际相关的那个。')
h('4.6 效率评估', 2)
p('表 5 报告 RTX 3090 上的效率（FP32、batch 1、640×640）。M5 以 117.6 FPS 为全场最快（图 4）——领先基线'
...
table(['模型', '参数量', 'GFLOPs', 'FPS（RTX 3090）'],
      ...
      '表 5 RTX 3090 效率（FP32、batch 1、640×640）。M5 兼具最少参数与最高实测吞吐；注意其 FLOPs '
      '与基线相当——收益在内存访问效率，这是 FLOPs 捕捉不到的。')
fig('results/analysis/fig8-pareto.png',
    '图 4 各检测器的精度-效率权衡（test mAP50 对参数量，对数轴）。标记大小正比于 RTX 3090 实测 FPS；'
    '空心标记为未测 FPS 的模型')
h('4.7 可视化分析', 2)
p('Grad-CAM 热力图（图 5a）显示基线与轻量变体的注意力都集中于猪体而非栏舍背景，且 FasterNet 替换'
  '并未明显移动关注区域——这与两模型在高频类上分类别表现一致相吻合。图 5b 给出一个典型残余错误：'
  '一头静止的卧猪被以高置信（0.84）误标为 fight，展示了密集场景中 fight/lying/紧密接触姿态之间的'
  '残余混淆，与归一化混淆矩阵（图 6）的主导误差一致。在个体分离良好的常规场景中，两个模型的检测结果几乎一致且正确；'
  '残余错误集中在拥挤、遮挡、身体纠缠的猪群——也正是人类观察者同样会犹豫的情形。')
fig('results/analysis/fig5-gradcam-hardcase.jpg',
    '图 5 Grad-CAM 对比与典型残余误检。(a) 原图 | 基线 | 轻量化变体：注意力集中于猪体而非栏舍背景，'
    '且经主干替换后保持。(b) 一头静卧猪被误标为 fight（0.84）——密集场景中 fight/lying/紧密接触混淆的'
    '典型案例，构成误差主体')
fig('results/m4-wsample/confusion_matrix_normalized.png',
    '图 6 M4 在验证集上的归一化混淆矩阵。误差质量集中于 fight、lying 与紧密接触姿态之间的混淆',
    width=5.4)
...
p('只能在服务器 GPU 上跑的检测器解决不了引言中的现场问题，因此我们在 Jetson Nano（4 GB、Maxwell GPU、'
  ...
  '链路为 PyTorch → ONNX（opset 12）→ TensorRT FP16（图 7）。导出件很小：M5 权重 5.2 MB'
  '（基线 5.5 MB）、ONNX 10.1 MB，栏端设备的远程更新成本极低。实测：M5 在 640×640 输入下每帧 50.2 ms'
  '（19.7 FPS），480×480 下 30.0 ms（33.3 FPS）——与基线（50.8 ms/19.7 FPS、29.9 ms/33.4 FPS）'
  '基本持平——持续满载下整机总输入约 7 W（空闲 3.4 W，GPU 与 CPU 计算轨合计约 5 W，实测见下）。两个工作点对行为监测都属实时'
  '（行为事件以秒级展开）；480×480 工作点还为同板多路视频流或多模型管线留出了余量。')
fig('results/analysis/fig9-deploy-pipeline.png',
    '图 7 从 PyTorch 权重到板上 TensorRT 推理的部署管线，含板上实测性能与实践中遇到的限制')
p('部署过程中有三点实践发现值得报告——这一领域文献很少记录它们，而每一点都真实消耗过我们的时间。'
  ...
  '（表 5）并未延续到边缘端。诚实的表述因此是：Nano 上打平、模型更小，而非速度领先。其三，构建工具链'
...
p('另外三组测量为"实时"二字划出精确边界。其一，延迟预算在 640×640 下分解得很干净：主机到设备'
  '传输 0.48 ms、GPU 计算 49.6 ms、设备回传 0.06 ms——引擎本身并不是需要盯防的瓶颈，CPU 侧才是：'
  '纯 Python 前/后处理管线的开销会超过 GPU 时间（640×640 图源的 JPEG 解码与缩放 17.1 ms、'
  '1080p 源 107.9 ms；优化 C 实现的归一化与排布转换 6.8 ms；约 370 候选的 NMS 仅 0.5 ms），'
  'TensorRT 的 enqueue 开销另有 9.5–10.4 ms，因此生产管线的前/后处理应当以 C 或 CUDA 实现。'
  '其二，持续运行稳定：连续 12,000 次推理（10.7 分钟）吞吐保持 20.0 FPS，第 99 百分位延迟 '
  '51.3 ms，GPU 温度峰值 55.5 °C，全程无热节流事件、无降频。其三，轨级功耗实测（INA3221）：'
  '空闲 3.4 W，持续满载总输入 ≈7.0 W，其中约 5.0 W 落在 GPU 与 CPU 轨——即上文引用的口径。'
  '延迟分解与持续运行时序见补充材料图 S2、图 S3。')
...
p('发布方切分把同一视频序列的帧分散到训练、验证、测试三集中。相邻帧近乎重复，因此上述主指标度量的是'
  ...
  '性能骤降（表 6，图 8）：基线在未见序列测试集上由 0.5964 跌至 0.155，在更难的验证集上跌至 0.075。'
...
table(['模型', 'val mAP50（混合异源，更难）', 'test mAP50（未见序列）'],
      [['基线', '0.075', '0.155'],
       ['M4（采样）', '0.139', '0.145'],
       ['M5（组合）', '0.107', '0.153']],
      '表 6 序列级互斥压力测试。所有模型相对随机切分都大幅退化；M4 将基线验证精度翻倍，'
      '但没有任何模型消除未见序列鸿沟。')
fig('results/analysis/fig8-generalization-ext.png',
    '图 8 框架的泛化行为。(a) 基线、M4、M5 在三种评测制度下的 mAP50：随机切分（test）、序列互斥'
    '压力测试与外部农场零样本评估。(b) 独立数据集零样本检测实况：白猪密集栏位大量漏检、花斑品种个体'
    '未检出、泥地场景部分检出——失效是环境层面与品种层面同时发生的')
h('6.2 跨数据集验证', 2)
p('更难的考验是换一个农场。我们将四个模型零样本（不做任何微调）在独立公开猪行为数据集 Comportamentos'
  '（696 张图像，CC BY 4.0 [26]）上评估——该数据集来自不同养殖场，地面材质、相机几何不同，且以花斑品种'
  '为主，而非源数据集的白猪。所有模型都跌至 0.036–0.067 mAP50（表 7），排序本身也富含信息：基线退化'
  '最轻（0.067），采样训练的模型退化最重（0.036–0.038）——再平衡使模型对其被平衡的源域更加特化。'
  'M5 的置信画像很有说服力：精确率 0.70、召回率 0.03，意味着模型在域外很少开口，一旦开口通常是对的'
  '——域外预测趋于保守而非随机。图 8b 让失效具体可见：白猪密集栏位大量漏检、花斑个体检不出、泥地'
...
table(['模型', 'mAP50', 'mAP50-95', '精确率', '召回率'],
      ...
      '表 7 Comportamentos 独立数据集零样本评估（696 张，CC BY 4.0，不同农场与品种）。退化是普适现象；'
...
p('纵观全文，有三点发现。第一，对不均衡行为数据，最便宜有效的干预在数据层：频率封顶复制不花任何参数'
  ...
  '打平——读者在类似板卡上应预期相同结果。'
  '与相邻路线的对照颇具教益。PBR-YOLO [27] 等注意力增强的轻量检测器报告了亮眼结果，但我们两次失败的'
  '集成（M1、M2）提示：把这类模块插入已预训练的通路时，其增益未必能迁移——恒等保持路线正是让主干'
  '替换在重度依赖迁移的管线中切实可行的关键。训练后压缩 [33] 与我们的数据层、主干层改动均正交，'
  '两条路线原则上可以叠加；这一组合留待未来工作。')
...
p('局限。最稀有类 sitting（144 实例）在所有模型与切分下仍然偏弱——解药是更多稀有类影像，而非更好的'
  ...
  '帧级瞬时的。我们实测了一种轻量时序后处理（IoU 轨迹匹配加多数类投票）：在验证集上没有精度收益——'
  '基准帧为稀疏抽样（相邻帧相差数十至数百帧），轨迹匹配跨时间间隔失效、投票反而污染正确的单帧标签。'
  '告警侧，帧级标签可自然聚合为栏级统计——打斗频次、静卧时长占比、饮水到访次数——'
  '其上以简单阈值规则即可触发福利告警；阈值整定因场而异，属于部署期工作。'
  '时序集成对密集连续帧的实时视频仍有希望——恰是第 5 节的部署形态——它与域适应、多农场数据采集'
  '并列为下一步方向。')
...
p('本文提出了一个轻量化、实时的群养猪多行为检测框架，将类别不均衡感知采样与 FasterNet 主干替换在'
  ...
  '（640×640 下 19.7 FPS、480×480 下 33.3 FPS），整机总功耗约 7 W。两级泛化分析——未见序列压力测试与'
  '独立农场数据集零样本评估——不加粉饰地划出了适用边界：框架今天即可用于同农场部署；域适应与多农场'
  '数据采集则是明天跨农场运行的关键方向。')

# ===== 声明 =====
h('声明', 1)
for line in ['作者贡献：Z.Z. 与 L.G. 对本工作贡献相同——构思、方法、软件实现、验证、形式化分析、'
             '实验调查、数据整理、初稿撰写、审阅修改与可视化。',
             '基金资助：本工作未获得专项资助。',
             '伦理委员会声明：不适用。本研究仅使用公开获取的非侵入式视频数据，未进行动物实验，无需伦理审批。',
             '知情同意声明：不适用。',
             '数据可用性声明：源数据集于 Roboflow Universe 公开（CC BY 4.0）[19]；外部验证数据集（Comportamentos）'
             '同样公开（CC BY 4.0）[26]。处理脚本、模型配置、训练日志与部署脚本已在 https://github.com/kksmks/pig-behavior-yolo 公开。',
             '利益冲突：作者声明无利益冲突。',
...
REFS = [
 ...
 'Luo, Y., Lin, K., Xiao, Z., et al.: PBR-YOLO: a lightweight piglet multi-behavior recognition algorithm '
 'based on improved YOLOv8. Smart Agric. Technol. 10, 100785 (2025). '
 'https://doi.org/10.1016/j.atech.2025.100785',
 'Rahman, M., Souza, V.H.S., Brown-Brandl, T.M., et al.: Accelerating sow nursing behavior monitoring with '
 'modified YOLO11n architecture and TensorRT integration. Porcine Health Manag. (2026). '
 'https://doi.org/10.1186/s40813-026-00507-3',
 'Yaman, B., Mahmud, T., Liu, C.H.: Instance-aware repeat factor sampling for long-tailed object detection. '
 'arXiv preprint arXiv:2305.08069 (2023)',
 'Crasto, N.: Class imbalance in object detection: an experimental diagnosis and study of mitigation '
 'strategies. arXiv preprint arXiv:2403.07113 (2024)',
 'Guo, A., Sun, K., Zhang, Z.: A lightweight YOLOv8 integrating FasterNet for real-time underwater object '
 'detection. J. Real-Time Image Process. 21(2), 49 (2024). https://doi.org/10.1007/s11554-024-01431-x',
 'Qin, C., Zhou, Z.: YOLO-FGD: a fast lightweight PCB defect method based on FasterNet and the '
 'Gather-and-Distribute mechanism. J. Real-Time Image Process. 21(4), 122 (2024). '
 'https://doi.org/10.1007/s11554-024-01504-x',
 'Luo, Y., Lin, K., Xiao, Z., et al.: Collaborative optimization of model pruning and knowledge distillation '
 'for efficient and lightweight multi-behavior recognition in piglets. Animals 15(11), 1563 (2025). '
 'https://doi.org/10.3390/ani15111563',
]
assert len(REFS) == 33
