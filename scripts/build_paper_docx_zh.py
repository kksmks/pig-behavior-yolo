# 组装中文版带图成稿 DOCX v4（与 JRTIP-paper-v4 逐节对齐：8 图 7 表 + 补充材料 Table S1）
# 运行：python scripts/build_paper_docx_zh.py → paper/猪行为检测-中文成稿-v4.docx
# v4 相对 v3：Table1+2 合并、部署表化正文、每类AP移补充材料、效率→表5/压力→表6/跨数据集→表7、
#             图5 Grad-CAM+难例左右拼、图8 泛化+异源上下拼、删每类AP柱状图、
#             Limitations 时序句改为实测阴性结果（2026-08-05 判决）
import sys
...
h('4.4 分类别分析', 2)
p('整体 mAP 掩盖了这套框架存在的理由，补充材料表 S1 因此在验证集上按行为类别拆分 AP50——'
