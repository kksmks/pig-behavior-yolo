...
## 2026-08-08（深夜④：中文版同步润色 6 处）

- 做了：
  - **中文 v5 同步 6/10 处**（build_paper_docx_zh_v5.py 改后重建）：摘要（三项设计选择/出人意料/收束闭环）、引言（工程趋势因此只有一个方向）、4.2（印证→支持）、4.3（值得记录→尤为突出）、**4.5（经不起审稿人推敲→会夸大证据——中文版同款元话语，必须同步）**、8 结论（诚实地→不加粉饰地）
  - 4 处不移植（中文原生自然、无对应毛病）：2.1 尾句、2.3"很少被讨论"、4.6"在意料之中"、5 部署（已是主动语态）
  - 重建校验：脚本内置 14 锚点断言通过、6 新文探针全中、6 旧文残留零
- 文件变更：scripts/build_paper_docx_zh_v5.py、paper/猪行为检测-中文成稿-v5.docx、STATE.md
- 卡点：无
- 用户侧剩余：通读 v5 四件（中英人味化已同口径）→ 填作者/单位/邮箱/ORCID + Funding + Author Contributions → 送导师 → 投稿
