...
| 时间戳 | 内容 |
|---|---|
| 2026-08-20 01:58 | **Zenodo DOI 落地（方案 A 完成）**：用户完成 Zenodo–GitHub 绑定 + v1.0.0 Release 发布（JRTIP submission snapshot），**Zenodo 存档 DOI = 10.5281/zenodo.22016701**——成果时间戳证据链闭环（git 历史 + CERN DOI 存档）；README 已挂 DOI badge。防占据三件套现状：Zenodo DOI ✅ / git 时间链 ✅ / 预印本（Research Square 或 arXiv，待作者信息填写后挂）⏳ |
| 2026-08-18 21:20 | **新建 PROJECT-MAP.md 项目结构导览**（用户要求）：树状结构+逐项注释（📄/🐍/📊/📦/⛔图例）、5 分钟上手路径（STATE→README→REVIEW_BOARD→EXPERIMENT_LOG）、易踩规矩备忘（英文稿补丁制/中文版整建/投稿包手动同步/git 先确认）——仓库新人与审稿人友好的结构说明，已入库 |
| 2026-08-18 21:00 | **图形升级二轮：figS1 升级 2×2 四模型 + JRTIP 格式统一**（用户开无卡机配合）：云端拉取 M3/M4 权重（无卡模式零成本），四模型 PR 本地复测全对（baseline 0.5963/M3 0.5691/M4 0.6035/M5 0.5933，与封账值一一吻合——M3 的 0.5691 与 7-26 补测完全一致）；figS1 重拼 2×2（(a)基线/(b)M3/(c)M4/(d)M5，M4 的 active 0.639 在 PR 曲线上一眼可见）；**Springer 格式统一**：裁掉 ultralytics 图内标题（图内不得放标题红线）、figS3 移除图内描述性标题、figS2 面板标题精简为纯 (a)/(b)、全部 600dpi；英文补充材料换图并清理 3 个孤儿图片关系（inline_shapes=3 干净）；ESM_1.pdf 重建；AutoDL 无卡机可关机 |
| 2026-09-08 | **TAHP 低级错误大扫除（用户要求）**：六维机检——刊名串台零（JRTIP 字样全清）/主稿完整（11 图 7 表、声明七件套、无 [n] 残留、零占位符）/补充材料页眉已换 TAHP/图件 8 张齐；**抓到 1 处**：CL 日期仍是 JRTIP 的 8 月 28 日（从老 CL 带的）→ 三处（md+双 docx）改 8 September 2026。页码修复版 TAHP-paper-v1.docx 待用户 Edit Submission 换文件+重贴 Comments |
| 2026-09-08 | **TAHP 技术退修秒修**：编辑室来信（JEO Assistant Varssha Rajaraman）仅要求一处——正文补连续页码（转版时只加了行号漏了页码，我的锅）；TAHP-paper-v1.docx 已加页脚 PAGE 域（居中页码）并导出验证（页码 2/3/4/5 连续、行号在、引用格式正常）；正式稿件号 **TROP-D-26-02281**（S 号为构建号）。待用户：EM 里 Edit Submission → Attach Files 换文件 → Build PDF → Approve |
| 2026-09-05 14:36 | 🎉 **TAHP 投稿正式完成**：稿件号 **TROP-S-26-02941**（Tropical Animal Health and Production，Springer，SCIE，订阅制免费）；EM 系统全流程走完（注册+ORCID+分类 Pig Production/Animal Production+文件 10 件+Comments 贴 CL+合成 PDF 两轮机检+Approve）；封面元数据标题连字符系 EM 解析缺陷（正文处处正确，不阻断）；Research Square 预印本已勾联动——转投闭环。后续：等技术审查/编辑分配邮件，预计一审 2–3 个月 |
| 2026-09-04 23:30 | **TAHP 转投版完成**（build_tahp_v1.py 由 v5 转出）：①引用体系数字制→作者-年份制（37 处正文点 + 33 条文献 Springer 作者-年份格式+字母序，Luo 同年双篇 a/b 后缀区分，数据集 Roboflow 2026a/b）②结论并入讨论章末（TAHP 结构）③新增 Statement of Animal Rights ④A4+双倍行距+连续行号（TAHP 排版要求）⑤Cover Letter 换刊重写（动物福利叙事优先，cover-letter-tahp.md/.docx）；投稿包 submission-package-tahp/（主稿+补充材料[页眉刊名已换]+ESM_1.pdf+CL+图件）——验证：无 [n] 残留、33 条字母序、结构探针全中 |
| 2026-09-04 | **JRTIP 桌拒（scope mismatch，非质量判决）+ 转投决策**：决定信（09-02）原文要点——编辑认定本刊核心是"实时图像处理本身"（复杂度/帧率/硬件实现），我们被读作畜牧应用论文，范围不合；无审稿意见、无技术问题。决策：不申诉（桌拒无翻盘）、不点付费转投流水线（推荐刊均 1–2 千欧金色 OA）；启用备胎梯队，**首选转投 Tropical Animal Health and Production（Springer/SCIE/订阅制免费/动物福利对口，框架零改动）**，梯队：Czech J Animal Sci（565 欧）→ Research in Agricultural Engineering（Diamond 免费）→ INMATEH（700 欧最快）。Research Square 预印本不受影响照常上线（成果保护完好）。待办：TAHP 投稿包（CL 换刊重写+指南微调） |
| 2026-08-28 18:48 | 🎉 **JRTIP 正式投稿完成**：SNAPP 系统全流程走完（Files 10 件/Details/Authors/Declarations/Review 全过），Research Square 预印本联动已勾选（投稿即公开 DOI 记录，成果保护闭环）；状态页：Submission received → Technical check 进行中；SNAPP 内部 ID 12f18e0f-516a-43f0-b511-a41a37267361，正式稿件编号待邮件。投稿前夜终检+当日自我审查全绿（图表三方对齐/声明七件套/作者四方一致/审计 25P0F0M）。后续：等邮件，技术审查 1–3 天，审稿周期约 100 天 |
| 2026-08-28 | **投稿日**：CL [Date] 填入 28 August 2026（md+双 docx+投稿包同步）——全部占位符清零，材料包进入可上传终态 |
| 2026-08-20 03:20 | **投稿前夜终检+漏洞修复**：全检八项通过（作者四处一致/邮箱统一/主稿零占位/审计 25P0F0M/页数 10/投稿包字节一致/数字三轮机检/相似性六组零碰撞）；**抓到唯一漏洞：中英补充材料页眉作者占位未填**（主稿审计只查主稿漏网，上传版 ESM_1.pdf 同样带占位）——已填（Zhan Zhang · Linlin Gou / 张展 · 勾林林 + 通讯邮箱）、中文重建、ESM_1.pdf 重导、投稿包同步；六文件复扫仅剩 CL [Date]（投稿当天填，设计内）。明日用户清单：CL 日期、Zenodo metadata 作者名、GitHub About、ORCID 可选、按六步走投稿 |
| 2026-08-20 02:55 | **相似性自查完成**（用户决定跳过付费 iThenticate，由助手网络撞库模拟）：6 组高风险原句（PConv 描述/不均衡三家分法/identity-preserving/onnxslim 排障叙事/摘要首句/frequency-capped）全部零逐字碰撞，报告 paper/similarity-check.md——编辑部正式查重风险低；推送积压已清（57bd767..b918cdd 已同步） |
| 2026-08-20 02:40 | 中文版作者名落汉字：张展†*、勾林林†（用户提供），通讯作者张展——中英双版作者信息全齐 |
| 2026-08-20 02:30 | **作者信息注入完成，占位符全清零**（用户决定暂不挂导师、双人共一独投 JRTIP）：Zhan Zhang†* & Linlin Gou†（共同一作），六盘水师范学院 生物科学与技术学院（用户机翻"biological schedule"已更正为官方口径 Biological Science and Technology），553000，通讯邮箱 851709772@qq.com；英文 v5 标题页+贡献声明（CRediT 全角色共一）+无资助声明+双人 bio（JRTIP 硬性要求）注入，Cover Letter 三处占位填写，中文版同步（拼音名，汉字待用户提供）；**页数复测仍 PAGES=10、合规审计 25P/0F/0M（MANUAL 项首次清零）**；审计脚本修 GBK 打印 † 崩溃 bug；journal-choice.md 更正 INMATEH 已收费（700 欧+超页 25 欧/页）——投稿包全同步，进入"随时可投"状态 |
| 2026-08-18 20:15 | **参照优秀论文升级图形（补充材料图 S1–S3，用户要求+杜绝学术不端：只学展示手法、数据全自有）**：figS1 双模型 PR 曲线并排（Tu et al. 手法；本地 CPU 产出 baseline 0.5963/M5 0.5933，与封账值 0.5964/0.5932 协议级一致✓）、figS2 延迟分解双联图（引擎堆叠条+CPU 侧对数条）、figS3 热节流时序（温度+利用率曲线，790 采样点）——全部进补充材料不占正文页；正文/中文各加两句指引（4.4→S1、部署段→S2/S3）；ESM_1.pdf 重建含三图；页数复测仍 PAGES=10 |
| 2026-08-18 19:40 | **排雷审计+唯一真雷修复**（用户要求）：全链路扫描——秘密零泄露/git 作者 noreply/CL 格式无损（新旧版 bold 均为 0）/STATE 91 条完整/REVIEW_BOARD 全标题/中英部署段数字一致/新数字与原始测量逐项一致/摘要 216 词/页数 10；**唯一真雷=fig9 部署管线图内"≈5 W"陈旧文本**（图文矛盾，build_figures.py:148 源头）——已改双口径"≈7 W sustained (≈5 W GPU+CPU rails); no INT8 on Maxwell"重出图、替换 EN v5 内嵌图（apply_v5_fig7swap.py 按 2.44 宽高比定位 rId17、字节级复检+抽图目检）、投稿包 Fig7 与 manuscript 同步、中文版重建嵌入新图、页数复测仍 PAGES=10；README 加草稿免责一行。draft-v1/v2 与日志旧记录属历史留痕非雷 |
| 2026-08-18 19:10 | **投稿包 README 版本注记同步**：头部加"内容同步：2026-08-18"及当日改动摘要（三段分解/功耗双口径/R2 两句/CL 同步），清单状态复核无误（实测页数已勾、仅剩 CL 日期=投稿日填）——投稿包文档体系自洽 |
| 2026-08-18 19:00 | **投稿包同步**：submission-package/manuscript 主稿还是 08-08 旧 v5（无三段分解/双口径/R2 句）、cover-letter 还是 08-06 旧版——已用当前版覆盖并机检（三段分解段✓ 双口径✓ 持续运行段✓ S2✓）；supplementary 无需动（08-07 后未改）——投稿包与仓库当前版完全一致 |
| 2026-08-18 18:50 | **Cover Letter 一致性修复 + 作者信息填空模板**：CL 三处同步（~5W→7W总输入/5W计算轨双口径、补"持续运行实测"段（12,000次/10.7min/零节流/CPU瓶颈指引）、补充材料 S1→S1+S2 双表），md 源与 docx 同步改、旧 5W 零残留；新建 paper/author-info-template.md（作者行/单位英文/ORCID/CRediT 贡献角色/Funding 二选一/bio 模板+填写清单）——用户剩余行政项变成填空题 |
| 2026-08-18 18:30 | **报告#8 残余项全部清零并推送**：①[31][32] 缺失性声明经 Semantic Scholar TLDR+摘要级双重核实成立（两篇均只论主干替换，未论及权重迁移；全文付费墙，残余风险与报告#7 一致）②R2-Minor 两处一句话入正文（3.1 标注协议出处 [20]、讨论章帧级→栏级统计→阈值告警路径，中英同步，apply_v5_r2minor.py 锚点断言）③页数复测仍 10/12 ④REVIEW_BOARD 增 #8-补（并修复昨日插入 #8 时误删的 #6 标题）——**报告#8 清单助手可办项全部清零，剩余均为用户项（作者信息/iThenticate/投稿）** |
